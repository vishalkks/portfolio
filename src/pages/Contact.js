...
function Contact() {
  ...
  const submitHandler = (e) => {
    ...
    const mailtoUrl = `mailto:vishalkks@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(`Email: ${email}\n\n${message}`)}`;
    const win = window.open(mailtoUrl, '_blank');
    win.focus();
    form.reset();
    toast.success('Awesome, fingers crossed you hit send in your mail client. Let\'s chat soon!', {
      position: "top-center",
      autoClose: 9000,
  ...
  return (
    <BlobsPlayground>
      ...
      >
        ...
        <div className="flex-1">
          <MessageInput name="message" placeholder="Say literally anything here..." />
          <Button label="Send" />
          <ToastContainer />
        </div>
      </motion.form>
